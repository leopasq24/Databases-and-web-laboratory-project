...
--
-- Host: 127.0.0.1
-- Creato il: Lug 25, 2023 alle 19:46
...
CREATE TRIGGER `limite_utente_standard` BEFORE INSERT ON `blog` FOR EACH ROW BEGIN
                   ...
                   IF (SELECT COUNT(blog.IdUtente) FROM blog WHERE NEW.IdUtente=blog.IdUtente)>=3 THEN SET NEW.IdUtente = null;
                  END IF;
                  END IF;
                  END
$$
...
--
DELIMITER $$
CREATE TRIGGER `scrittura_blog` BEFORE INSERT ON `post` FOR EACH ROW BEGIN
          IF NEW.IdUtente NOT IN (SELECT blog.IdUtente FROM blog WHERE NEW.IdBlog = blog.IdBlog)
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `scrittura_coautore` BEFORE INSERT ON `post` FOR EACH ROW BEGIN
          IF NEW.IdUtente NOT IN (SELECT coautore.IdUtente FROM coautore WHERE NEW.IdBlog = coautore.IdBlog)
                  THEN SET NEW.IdUtente = null;
                  END IF;
                  END
$$
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `numeroblog`  AS SELECT `IdUtente` AS `IdUtente`, count(`IdBlog`) AS `NumeroBlog` FROM `blog` GROUP BY `IdUtente``IdUtente`  ;
