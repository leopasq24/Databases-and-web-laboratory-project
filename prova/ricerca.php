...
if($selezione == "blog" and $str!=""){
    $new_str="%".$str."%";
    $stmt= mysqli_prepare($link, "SELECT DISTINCT blog.IdBlog AS idblog, Titolo, Descrizione, Nome, Username FROM blog, categoria, utente WHERE blog.IdCategoria=categoria.IdCategoria AND blog.IdUtente=utente.IdUtente  AND (Titolo LIKE ? OR Descrizione LIKE ? OR Nome LIKE ? OR Username LIKE ?)");
    ...
    $new_str="%".$str."%";
    $stmt= mysqli_prepare($link, "SELECT DISTINCT Titolo, Testo, Username FROM post, utente WHERE post.IdUtente=utente.IdUtente  AND (Titolo LIKE ? OR Testo LIKE ? OR Username LIKE ?)");
